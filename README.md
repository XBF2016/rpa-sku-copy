...
### 操作前预滚动（更贴近人工视角）
- 页面就绪后：程序会自动将视图滚动到“添加规格类型”按钮附近，便于后续操作区始终在屏幕可见范围。
- 维度与选项录入：在对某个维度开始输入前，会将页面滚动到该维度容器附近（保留上边距约 120px），再进行输入与回车提交。
- 保存草稿：在点击“保存草稿”按钮前，同样会先将视图滚动到按钮附近，避免页面在未对齐时点击失败。
- 以上滚动均基于元素的相对位置计算，默认降级为 `scrollIntoView({block:'center'})`，不硬编码任何不稳定的类名或后缀。
